...
def create_missing_tables(project_id: str, dataset_id: str, omop_version: str) -> None:
    ...
    try:
        ...
        create_no_return = create_sql.replace('\n', '')
        utils.logger.warning(f"create SQL is {create_no_return}")
        ...
        if create_statement != "":
            try:
                ...
                with conn:
                    ...
                    """
                    sql_no_return = sql_statement.replace('\n','')
                    utils.logger.warning(f"dervied SQL is {sql_no_return}")
...
def placeholder_to_table_path(site: str, site_bucket: str, delivery_date: str, sql_script: str, vocab_version: str, vocab_gcs_bucket: str) -> str:
    ...
    for placeholder, replacement in constants.VOCAB_PATH_PLACEHOLDERS.items():
        ...
        #vocab_table_path = f"gs://{site_bucket}/{delivery_date}/{constants.ArtifactPaths.CONVERTED_FILES.value}{replacement}{constants.PARQUET}"
