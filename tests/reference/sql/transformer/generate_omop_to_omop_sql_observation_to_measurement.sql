            ...
            COPY (
                SELECT
 CAST(COALESCE(hash(CONCAT(CAST(observation_id AS VARCHAR),CAST(person_id AS VARCHAR),CAST(observation_concept_id AS VARCHAR),CAST(observation_date AS VARCHAR),CAST(observation_datetime AS VARCHAR),CAST(observation_datetime AS VARCHAR),CAST(observation_type_concept_id AS VARCHAR),CAST(0 AS VARCHAR),CAST(value_as_number AS VARCHAR),CAST(COALESCE(vh_value_as_concept_id, value_as_concept_id) AS VARCHAR),CAST(unit_concept_id AS VARCHAR),CAST(NULL AS VARCHAR),CAST(NULL AS VARCHAR),CAST(provider_id AS VARCHAR),CAST(visit_occurrence_id AS VARCHAR),CAST(visit_detail_id AS VARCHAR),CAST(observation_source_value AS VARCHAR),CAST(observation_source_concept_id AS VARCHAR),CAST(unit_source_value AS VARCHAR),CAST(0 AS VARCHAR),CAST(value_source_value AS VARCHAR),CAST(observation_event_id AS VARCHAR),CAST(obs_event_field_concept_id AS VARCHAR),'test_site')) % 9223372036854775807, '-1') AS BIGINT) AS measurement_id,
 ...
 TRY_CAST(0 AS BIGINT) AS operator_concept_id,
 TRY_CAST(value_as_number AS DOUBLE) AS value_as_number,
 TRY_CAST(COALESCE(vh_value_as_concept_id, value_as_concept_id) AS BIGINT) AS value_as_concept_id,
 ...
 TRY_CAST(observation_source_concept_id AS BIGINT) AS measurement_source_concept_id,
 TRY_CAST(unit_source_value AS VARCHAR) AS unit_source_value,
 TRY_CAST(0 AS BIGINT) AS unit_source_concept_id,
 ...
 TRY_CAST(obs_event_field_concept_id AS BIGINT) AS meas_event_field_concept_id
