SELECT
            (SELECT COUNT(*) FROM (
             ...
             ) c WHERE c.row_hash NOT IN (SELECT row_hash FROM read_parquet('gs://test-bucket/2025-01-15/artifacts/post_processing/remove_test_patients/tmp/death_pre.parquet'))) AS added,
            (SELECT COUNT(*) FROM read_parquet('gs://test-bucket/2025-01-15/artifacts/post_processing/remove_test_patients/tmp/death_pre.parquet') s
